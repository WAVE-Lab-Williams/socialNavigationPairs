...
function loadInstrContent() {
    var instrContent = [
        ...
        /* ind1 */ `<p>In this experiment, you will see an image of two people wearing <b>striped shirts</b> flash on your screen.<p>Your task is to determine if the two people are wearing exactly the same shirt!</p><p> Below is an example of <b> shirts that are <i>not</i> identical</b>. Although the angle of the stripes are similar, they are not the same.</p><div style='text-align: center'><img src="${stimFolder}person_example_different.png" style="width: 300px;"/></div>`,
        /* ind2 */ `<p>Below is an example of <b> shirts that are identical</b>. Note that the angle of the stripes are <i>exactly</i> the same.</p><div style='text-align: center'><img src="${stimFolder}person_example_same.png" style="width: 300px;"/></div>`,
        /* ind3 */ `<p>To make it more difficult, we've placed the two people in striped shirts in a room full of other people with different color shirts and different colorful things. People in the room may appear at different times.</p><div></div><p>For this experiment, though, <b>you should only care about the people with yellow striped shirts.</b></p><p>When you think the two people are wearing the <i>same</i> shirt, press the <b>f key</b>. When you think the two people are <i>not</i> wearing the same shirt, press the <b>j key.</b></p>`,
        /* ind4 */ `<p>Sometimes, you may be unsure of your answer. This is okay, since the task is designed to be tricky! In times like this, just give it your <b>best guess</b>, as we are interested in your <i>gut intuition</i>.</p><p>Please do your very best to remain as focused and attentive as possible, even at the end of the experiment. We know it is very difficult to stay focused for so long, especially when you are doing the same thing over and over. But remember, the experiment will be all over in less than ${String(estTotalRunTime)} minutes. Press the button below to begin.`,
    ];
